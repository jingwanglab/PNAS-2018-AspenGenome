...
#SBATCH -A b2010014
#SBATCH -p core
#SBATCH -o est_dfe.summary.expression.SwAsp94.out
#SBATCH -e est_dfe.summary.expression.SwAsp94.err
#SBATCH -J est_dfe.summary.expression.SwAsp94.job
#SBATCH -t 1-00:00:00
...
OutDir="/proj/b2010014/nobackup/population_genetics/DFE_expression/Out/$class/$level/$anno/summary"

if [ ! -d "$OutDir" ]; then
mkdir -p $OutDir
fi

echo -e "Name\tN2\tt2\tNw\tb\tEs\tf0\tL\tNes_1\tNes_1_10\tNes_10_100\tNes_100\tlambda\tselected_divergence\talpha\tomega_A" > $OutDir/tremula.dfe.$class.$level.$anno.summary

#####the real data
Input_real="/proj/b2010014/nobackup/population_genetics/DFE_expression/Out/$class/$level/$anno/real"
...
lambda=$(cut -f 2 -d " " $Input_real/est_alpha_omega.$anno.$class.$level.out)
selected_divergence=$(cut -f 4 -d " " $Input_real/est_alpha_omega.$anno.$class.$level.out)
alpha=$(cut -f 6 -d " " $Input_real/est_alpha_omega.$anno.$class.$level.out)
omega_A=$(cut -f 8 -d " " $Input_real/est_alpha_omega.$anno.$class.$level.out)

echo -e "real\t$N2\t$t2\t$Nw\t$b\t$Es\t$f0\t$L\t$Nes_1\t$Nes_1_10\t$Nes_10_100\t$Nes_100\t$lambda\t$selected_divergence\t$alpha\t$omega_A" >> $OutDir/tremula.dfe.$class.$level.$anno.summary

###bootstrap

for n in {1..200}
do
Input_bootstrap="/proj/b2010014/nobackup/population_genetics/DFE_expression/Out/$class/$level/$anno/bootstrap/bootstrap$n"
...
lambda=$(cut -f 2 -d " " $Input_bootstrap/est_alpha_omega.$anno.$class.$level.out)
selected_divergence=$(cut -f 4 -d " " $Input_bootstrap/est_alpha_omega.$anno.$class.$level.out)
alpha=$(cut -f 6 -d " " $Input_bootstrap/est_alpha_omega.$anno.$class.$level.out)
omega_A=$(cut -f 8 -d " " $Input_bootstrap/est_alpha_omega.$anno.$class.$level.out)

echo -e "bootstrap$n\t$N2\t$t2\t$Nw\t$b\t$Es\t$f0\t$L\t$Nes_1\t$Nes_1_10\t$Nes_10_100\t$Nes_100\t$lambda\t$selected_divergence\t$alpha\t$omega_A" >> $OutDir/tremula.dfe.$class.$level.$anno.summary
