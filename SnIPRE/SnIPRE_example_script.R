...
setwd("~/Dropbox/aspen_genome_paper/data/SnIPRE_SwAsp94samples/SnIPRE_code_JAGS")
...
data <- read.table("SwAsp_94samples.SnIPRE.out.txt", header = TRUE)  # sample data set
...
write.table(res, file = "SnIPRE.SwAsp.94samples.eb_results.csv", sep  = ",", row.names = FALSE)
write.table(res, file = "SnIPRE.SwAsp.94samples.eb_results.txt", sep  = "\t", row.names = FALSE,col.names=T,quote=F)
...
BSnIPRE.run(data, burnin = 10000, thin = 4, iter = 100000)
...
write.table(bres, file = "SnIPRE.SwAsp.94samples.bayesian_results.txt", sep  = "\t", row.names = FALSE,col.names=T,quote=F)
